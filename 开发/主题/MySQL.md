...
- truncate table test 执行更快，清空表中的所有内容
- delete from test 按行删除，而且可以通过 where 语句选择要删除的行
...
横表指的是将属性作为列，也是平时大部分时候的用法；竖表指的是将属性作为“属性名”列的值这种组织方式。竖表可以灵活、动态的加新属性，如果每种属性的记录数量不一样，用竖表会更节省空间。
...
可重复读，可避免脏读、不可重复读的发生（MySQL InnoDB 默认事务隔离级别，Oracle 默认采用的读已提交）
读已提交，可避免脏读的发生
读未提交，最低级别，任何情况都无法保证
...
如果事务 A 按一定条件搜索， 期间事务 B 删除了符合条件的某一条数据，导致事务 A 再次读取时数据少了一条，这种情况归为不可重复读。
...
```shell
mysql> set global sort_buffer_size =131072; # 单位为B，即 128KB，默认 64K
```
...
### float 和 double
float 最多可以存储 8 位的十进制数，并在内存中占 4 字节，double 最多可以存储 16 位的十进制数，并在内存中占 8 字节。
...
存储过程是一些预编译的 SQL 语句，可以降低网络通信量，提高通信效率，但是从版本管理的角度看很不友好。
